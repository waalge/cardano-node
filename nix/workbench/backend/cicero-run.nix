...
let
  inherit (cicero-workbench) workbench backend cacheDir stateDir basePort;
  ...
  inherit (with-cicero-profile {}) profileNix profile topology genesis;
in
  let
    ...
    profile-run =
      ...
      let
        ...
        run = pkgs.runCommand "workbench-run-cicero-${profileName}"
...
{
  inherit stateDir;
  inherit workbench cicero-workbench;
