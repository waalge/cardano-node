usage_cicero() {
     usage "cicero" "Backend:  manages a local cluster using 'cicero'" <<EOF
...
case "$op" in
    ...
    setenv-defaults )
        local usage="USAGE: wb cicero $op PROFILE-DIR"
    ...
    allocate-run )
        local usage="USAGE: wb cicero $op RUN-DIR"
    ...
    service-up )
        local usage="USAGE: wb cicero $op RUN-DIR NODE-NAME"
        local dir=${1:?$usage}; shift
    ...
    start-nodes )
        local usage="USAGE: wb cicero $op RUN-DIR [HONOR_AUTOSTART=]"
        local dir=${1:?$usage}; shift
    ...
    # Docker-specific
    is-service-running )
        local usage="USAGE: wb cicero $op RUN-DIR DOCKER-SERVICE"
        local dir=${1:?$usage}; shift
        local service=${1:?$usage}; shift
    ...
    wait-node-stopped )
        local usage="USAGE: wb cicero $op RUN-DIR NODE"
        local dir=${1:?$usage}; shift
        local node=${1:?$usage}; shift
